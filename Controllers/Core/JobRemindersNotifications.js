...
const Mailer = require('../../Controllers/Core/Mailer');
var FCM = require('fcm-push');

module.exports.jobRemindersNotifications = function jobRemindersNotifications(userID,data,title,name,isCustomer) {
    ...
    getSettingsQuery.then((settings) => {
        ...
        if (settings.notifications.transactional.Email == true) {
            ...
            if(isCustomer){
                Mailer.sendJobRemainderToCustomer("Job Reminder", userID,title,name);
            }else{
                Mailer.sendJobRemainderToProvider("Job Reminder", userID,title,name);
            }
        }
